...
// `opts.icons` overrides the font check. The browser leaves it unset and the gate reads the font;
// the server, which has no fonts, is TOLD by the browser whether the icon font arrived, and the
// sheet it generates — and later regenerates to check an answer — has to depend on that answer
// and nothing else.
export function generateQuestion(bandKey, type, seed, opts = {}) {
  ...
  const available = [
    ...
    { types: (opts.icons ?? iconFontReady()) ? band.iconTypes : [], weight: band.sources.icon },
